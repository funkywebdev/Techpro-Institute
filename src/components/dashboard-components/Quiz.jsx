...
//     // 👇 LOG JUST THE DATA (most important)
//     console.log("RESPONSE DATA 👉", res.data);

//     setResult(res.data.data);
//     setShowResult(true);
//   } catch (error) {
//     // 👇 VERY IMPORTANT: log error details
...
const Quiz = () => {
  ...
  if (!questions.length) return <p className="text-center mt-10 text-gray-700">No quiz available</p>;
  ...
  return (
    <div className="flex items-center justify-center px-4 bg-gray-100">
      <div className="w-full max-w-6xl bg-white shadow-md rounded-xl p-6">
        ...
        {!quizStarted && (
          <div className="text-center py-16">
            <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800">Get Ready!</h1>
            <p className="text-sm sm:text-lg mb-6 text-gray-600">Quiz starts in</p>
            <div className="text-4xl sm:text-6xl font-extrabold text-gray-900">{readyTime}</div>
          </div>
        )}
        ...
        {quizStarted && !showResult && (
          <>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
              <div className="bg-[#15256E] h-2 rounded-full transition-all" style={{ width: `${progress}%` }} />
            </div>
            ...
            {/* QUESTION */}
            <h2 className="text-lg sm:text-xl font-semibold mb-6 text-gray-800">{currentQ.question}</h2>
            ...
            <div className="space-y-3">
              {currentQ.options.map((option, i) => (
                ...
                >
                  <span className="mr-3 font-semibold">{optionLabels[i]}</span>
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={handlePrev}
                disabled={currentQuestion === 0}
                className="flex-1 py-2 rounded-lg bg-gray-200 text-gray-600 disabled:opacity-50"
              >
                ← Prev
              </button>
              <button
                onClick={() => handleNext(false)}
                disabled={!selectedOption || submitting}
                className="flex-1 py-2 rounded-lg bg-[#15256E] text-white disabled:opacity-50"
              >
        ...
        {showResult && (
          <div className="text-center py-10">
            <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800">🎉 Quiz Completed</h2>
            <div className="text-4xl sm:text-5xl font-extrabold text-[#15256E] mb-3">
              {result?.score}/{questions.length}
            </div>
            ...
            <button
              onClick={handleRestart}
              className="px-6 py-2 rounded-lg border-2 border-[#15256E] hover:bg-[#15256E] hover:text-white"
            >
              Retry
            </button>
          </div>
