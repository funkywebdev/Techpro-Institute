...
const Sidebar = () => {
  ...
  return (
    <aside className="w-56 sm:w-64 min-h-screen bg-[#111827] text-gray-200 flex flex-col items-center py-5">
      ...
      {/* Logo / Profile */}
      <div className="flex flex-col items-center w-full gap-1 py-4 text-center border-b border-gray-600 sm:gap-2 sm:py-1">
        <img
          src={techprologo}
          alt="TechPro Logo"
          className="rounded-full w-28 sm:w-40 -translate-x-9 sm:-translate-2"
        />
        ...
        {student?.profilePicture?.url ? (
          <img
            ...
            alt="Profile"
            className="object-cover w-16 h-16 mb-2 -translate-x-12 rounded-full sm:w-20 sm:h-20 sm:-translate-7"
          />
        ) : student ? (
          <div className="w-16 sm:w-20 h-16 sm:h-20 rounded-full bg-[#F81C0D] mb-2 flex items-center justify-center text-white font-bold text-lg -translate-x-12 sm:-translate-7">
            {getInitials(student.firstName, student.lastName)}
          </div>
        ) : (
          <div className="w-16 h-16 mb-2 -translate-x-12 bg-gray-700 rounded-full sm:w-20 sm:h-20 sm:-translate-7" />
        )}

        {/* Student Name */}
        <p className="text-xs font-normal text-white sm:text-sm -translate-x-11 sm:-translate-6">
          {loading
            ? "Loading..."
            : student
      ...
      {/* Menu */}
      <ul className="flex flex-col items-center flex-1 w-full px-2 py-2 space-y-5 sm:px-4 sm:py-4 sm:space-y-3">
        {menuItems.map((item) => (
      ...
      {/* Logout */}
      <div className="flex justify-center w-full py-3 border-t border-gray-600 sm:py-4">
        <div
