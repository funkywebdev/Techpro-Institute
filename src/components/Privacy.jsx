...
const sections = [
  { id: "Information We Collect", title: "Personal Information", content: <p>Name, email, phone number, and other details provided during registration or inquiries. Browser type, device information, and website usage data collected through cookies or analytics tools.</p> },
  { id: "How We Use Your Information", title: "How We Use Your Information", content: <p>1. To provide and improve our services.<br/>2. To communicate with you regarding updates, promotions, or inquiries.<br/>3. To ensure website security and functionality.</p> },
...
const Privacy = () => {
  ...
  return (
    <div id="privacy" className="bg-[#f7f9fc] min-h-screen">
      ...
      <div className="bg-gradient-to-br from-[#15256E] to-[#1f3aa8] text-white relative overflow-hidden">
          ...
          <ul className="mt-2 bg-white rounded-xl shadow p-3 space-y-1">
            {sections.map((s, index) => (
              <li
                ...
                className={`cursor-pointer transition px-4 py-2 rounded text-sm ${
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-12 grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-12">
        ...
        <main className="lg:col-span-3 space-y-4 md:space-y-8">
          {sections.map((section, index) => (
            <section key={section.id} ref={sectionRefs.current[index]} className="bg-white rounded-2xl shadow-md p-4 md:p-6 hover:shadow-lg transition">
